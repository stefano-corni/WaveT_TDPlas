...
!        use global_quantum, only: quantum_Ffld   !serve per i test
        ...
        public read_input_tdplas, read_input_tdplas_for_propagation

        contains

        subroutine read_input_tdplas(calculation_exe, user_input)
            ...
            select case(calculation_exe)
                case("tdplas")
                    user_input%surface_type    = "object"
                ...
                case("frequency")
                    ...
                    call read_ext_pert_nml(user_input)
