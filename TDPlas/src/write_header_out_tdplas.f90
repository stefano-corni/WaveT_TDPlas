   ...
   subroutine write_out(calculation_exe)
       ...
       tp_myrank=0
                ...
                if (tp_myrank.eq.0) then
                    ...
                    call quantum_coupling_write
                    if(calculation_exe.eq."frequency") &
                       call ext_pert_write
                    call out_write
                endif
    end subroutine
...
!------------------------------------------------------------------------
       ...
       select case (global_medium_Fpol)
            ...
            select case(global_medium_read_write)
                case ('rea')
                    write(6,*) "This is full run reading matrix and boundary"
                case ('wri')
            ...
            select case(global_medium_Fnorm)
                case ("tot") 
                    write(6,*) "Normalization of charges over total tesserae"
                case ("sep")
                    write(6,*) "Normalization of charges on different sets of tesserae"
                case ("non")
                    write(6,*) "Charges are not normalized" 
            end select
            select case(global_medium_bem_sym)
                case ("yes")
                    write(6,*) "Diagonalization of symmetric matrix S^-1/2DAS^1/2+S^1/2AD*S^-1/2"
                case ("non")
                    write(6,*) "Diagonalization of non-symmetric matrix DA"
            end select
    end subroutine
    ...
    subroutine poles_write()
        if (global_eps_Feps.eq."gen") then
                select case(typ_prop)
                     case("0")
                             write(6,*) "Propagation of last pole with velocity-verlet algorithm"
                     case("1")
                             write(6,*) "Propagation of last pole with first order algorithm"
                     case("2")
                             write(6,*) "Propagation of last pole with inversion algorithm"
                     case("3")
                             write(6,*) "Propagation of last pole with second order algorithm"
                end select
        endif
    end subroutine

    subroutine quantum_coupling_write()
        if(global_qmodes_nprint.eq.pedra_surf_n_tessere) then
            write(6,*) "all modes diagonalized and printed "
        else if (global_qmodes_nprint.gt.0) then
            write(*,*) global_qmodes_nprint," quantum plasmonic mode will be",&
                                    "printed"
        end if
        ...
        if (global_surf_Fsurf.eq.'sphe') then
            ...
            select case(pedra_surf_Fcav)
                ...
                case ('gms')
                    ...
                    select case(pedra_surf_Ffind)
                        case ('yes')
                            write(6,*) 'Spheres will be found from gmsh file'
                        case ('non')
                             write(6,*) 'Spheres will not be found from gmsh file'
                    end select
                case ('bui')
                    write(6,*) "Building surface from spheres."
                    write(6,*) "Cavity tessere number", pedra_surf_n_tessere
                    !non stampa più nell'output a geometria della cavità, va bene lo stesso?
            end select
       endif
    end subroutine
...
! EC 281117: added restart for medium
        select case(global_prop_Fmdm_res)
            ...
            write(*,*) 'Frequency of updating the interaction potential: ', global_prop_n_q
...
!               are calculated for the ground state
    ...
    subroutine ext_pert_write()
       select case(global_ext_pert_Ftyp)
            case ('field')
                write(6,*) "The applied perturbation is an external field directed as"
                write(6,*) global_ext_pert_direction
            case ('molecule')
                write(6,*) "The applied perturbation is an extenal potential read from"
                write(6,*) "ci_trans.inp, for example the transition potential from a molecule"
                if (global_ext_pert_Feet.eq."yes") then
                  write(6,*) "ci_trans_2.inp will also be read to perform a eet calculation"
                endif
            case('from_cipot')
                write(6,*) "The applied perturbation is an extenal potential read from"
